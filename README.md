🌟 Web Development Portfolio - Ray Tian  
  
Welcome to my Web Development Portfolio, this is for showcasing my skills and projects in web development. This repository demonstrates my technical abilities and problem-solving skills across various web technologies and other projects.  
  
📘 Portfolio Overview  
  
🧠 This portfolio highlights my expertise in:  
- Frontend web development with responsive design  
- Technical programming through other projects  
- Interactive web features and animations  
  
🗂️ Key Components  
  
✅ 🌐 Portfolio Website  
- Personal introduction and skills showcase  
- Responsive design with modern CSS techniques  
- Interactive elements and animations  
  
✅ 🎮 CS2 Game Analysis
- Memory structure documentation  
- Offset mapping for game data  
- ESP functionality implementation  
  
✅ 🌱 Global Goals Project 
- Climate action initiatives  
- Sustainable development contributions  
  
✅ ♿ Accessibility Features 
- Inclusive design principles  
- Accessibility implementation showcase
  
✅ 🎵 Music Hobby
- Explain my hobby of music
- Sharing my original song
- Music arrangement and recording practice
  
✅ 🔐 AES Tool
- Data encryption and decryption tool based on AES algorithm
- User-friendly encryption interface
- Supports different key lengths
  
💻 Technical Implementation  
  
🔍 CS2 Analysis Components 
- `client_dll.hpp`: Game structure definitions  
- `offsets.hpp`: Memory address mapping  
- ESP visualization implementation  
  
📞 Contact Information  
  
- GitHub: [1dongguo1](https://github.com/1dongguo1)  
- Bilibili: [Ray's Channel](https://space.bilibili.com/597194489)  

⚠️ Important Notes 
Memory offsets require regular updates when the game is patched.

[![Ask DeepWiki](https://deepwiki.com/badge.svg)](https://deepwiki.com/1dongguo1/WebDev-Portfolio--Ray-Tian)

<!-- ### This is for DeepWiki to update -->